...
class LSTMDataset(Dataset):
    def __init__(self, data_path, max_len, bert_config,modelPATH):
        ...
        self.max_counts = 0;
        with torch.no_grad():
            for inputs, mask, segment, target, text in tqdm(loader,desc='model_output',leave=False):
    ...
    def __getitem__(self, i):
        return self.hiddens_tensor[i], self.target_list[i], self.counts_list[i]
...
def collate_LSTM(batch):
    ...
    hiddens_tensor,target, length = map(
        lambda x: x.to(device),
        (hiddens_tensor, target, length),
    )

    return hiddens_tensor, target, length
