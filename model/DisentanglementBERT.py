...
class BertForToefl(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
                position_ids=None, head_mask=None, inputs_embeds=None, labels=None, labels_prompt=None, mode='bert'):
        ...
        if mode == 'bert':
            logits_L1 = self.L1Classifier(pooled_output)
            logits_prompt = self.PromptClassifier(pooled_output)
            outputs = (logits_L1, logits_prompt, ) + outputs[2:]  # add hidden states and attention if they are here

            # calculating loss
            loss_fct = CrossEntropyLoss()
            adversarial_loss_fct = HLoss()
            loss = loss_fct(logits_L1.view(-1, self.num_labels), labels.view(-1)) + 0.1*adversarial_loss_fct(logits_prompt.view(-1, self.num_prompt))
